...
export const CharacterMessageWrapper = styled.div<{
    ...
    max-width: 100%;
    align-items: flex-start;
    justify-content: ${(props) => (props.sentby === 'user' ? 'flex-end' : 'flex-start')};
    padding-left: ${(props) => (props.sentby === 'character' && !props.showProfile ? '3.2rem' : '0')};
...
export const UserMessageWrapper = styled.div<{
    ...
    width: 100%;
    margin-left: ${(props) => (props.sentby === 'character' && !props.showProfile ? '3.2rem' : '0')};
...
export const Message = styled.div<{
    ...
    padding: 1rem;
    border-radius: 10px;
    max-width: ${(props) => (props.sentby === 'user' ? '70%' : 'calc(100% - 4rem)')};
    overflow-wrap: break-word;
    margin-bottom: ${(props) => (props.showMessageTime === false ? '0.4rem' : '0.75rem')};
    margin-top: ${(props) => (props.showProfile === true ? '0.5rem' : '0')};
    ${(props) => (props.sentby === 'user' ? userMessageStyles : characterMessageStyles)};
    background-color: ${(props) => (props.sentby === 'user' ? '#6d2fef' : props.isReply ? '#e5ddf5' : '#fcfcfc')};
`;
...
export const MessageImage = styled.img<{
    ...
    max-height: 20rem;
    border-radius: 10px;
    margin: ${(props) => (props.showMessageTime === false ? '0 0.3rem 0.4rem 0.75rem' : '0 0.3rem 0.75rem 0.75rem')};
...
export const SubContainer = styled.div`
    height: calc(100vh - constant(safe-area-inset-top) - constant(safe-area-inset-bottom) - 10rem);
    height: calc(100vh - env(safe-area-inset-top) - env(safe-area-inset-bottom) - 10rem);
