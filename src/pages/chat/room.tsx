    ...
    return (
        <S.CharacterMessageWrapper sentby="character" showProfile={showProfile} showChatTutorial={showChatTutorial}>
            ...
            <S.MessageDiv showProfile={showProfile}>
                ...
                <S.MessageContent>
                    {message.type === 'text' ? (
                        <S.Message showProfile={showProfile} showMessageTime={showMessageTime} sentby="character">
                            {message.content}
                        </S.Message>
                    ) : (
...
export const ChatRoom = (): JSX.Element => {
    ...
    useEffect(() => {
        ...
        const timeStamp = new Date(Date.parse(nowTime) + koreaTimeOffset).toISOString();
            ...
            if ('reply' in i && i.reply) {
                ...
                if (fiveMinPlus <= new Date().getTime()) {
                    i.reply.forEach((text) => {
                        const afterFiveM = new Date(Date.parse(i.timeToSend) + 5 * 60 * 1000 + koreaTimeOffset)
                            .toISOString()
                            .replace('T', ' ')
                            .substring(0, 16);
    ...
    const checkOverScroll = (e: any) => {
        ...
        if (isTuto !== true) {
            ...
            if (current) {
                ...
                if (e.target.scrollTop > 0 && isScrollTrigger === true && isLastChat === false) {
                    // 아래 주석으로 교체할 것.
