...
export const Create = () => {
  ...
  async function requestPermissionsOrUpload(event: any) {
        ...
        return new Promise((resolve) => {
          const handlePermissionMessage = (event: any) => {
            try {
              ...
              if (
                data.cameraPermission !== undefined &&
                data.libraryPermission !== undefined
              ) {
    ...
    const {
      cameraPermission,
      libraryPermission,
    }: { cameraPermission: boolean; libraryPermission: boolean } =
    ...
    <S.Container>
      ...
      <S.SubContainer className="container">
        ...
        <S.ImageContainer>
          <S.InputWrapper>
            <S.InputTitle>
              캐릭터 이미지 (1개 이상)<S.RedText>*</S.RedText>
            </S.InputTitle>

            <S.DeleteButton onClick={handleDeleteImages}>
              전체 삭제
            </S.DeleteButton>
          </S.InputWrapper>
          ...
          <S.ImageLabel
            ...
            onClick={(event) => requestPermissionsOrUpload(event)}
          >
            + 이미지 업로드
          </S.ImageLabel>
          <S.ImageInput
        ...
        <StyledInput
          placeholder="등록심사 결과를 안내 받을 이메일 주소를 입력해주세요"
          content={email}
          setContent={setEmail}
          limit={50}
          height="5.1rem"
