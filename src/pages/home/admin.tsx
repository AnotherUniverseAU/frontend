...
export const Admin = () => {
    ...
    useEffect(() => {
        ...
        getCharInfo();
        setTimeValue(new Date(new Date().getTime() + 9 * 60 * 60 * 1000).toISOString().slice(0, 10));
    }, []);
    useEffect(() => {
        if (isAdmin) {
    ...
    const onClick = (e: any) => {
        ...
        if (charIdCurrent && DateCurrent) {
            ...
            apiRequestGet(`/chatroom/chat-history/${charId}/${timestamp}?offset=0`).then((res) => {
                console.log(res);
                                ...
                                <Tr key={'d' + String(idx)}>
                                    {chat.imageUrl ? (
                                        <ContentTd key={'i' + String(idx)}>
                                            <img src={chat.imageUrl} alt="이미지" style={{ maxWidth: '50%' }}></img>
                                        </ContentTd>
                                    ) : (
